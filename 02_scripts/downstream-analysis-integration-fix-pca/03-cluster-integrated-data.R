...
# read in data
integrated_seurat <- readRDS("../../results-updated-20251128/02_integration/integrated_seurat_object.Rds")
...
cells_per_cluster <- metadata %>% group_by(integrated_snn_res.0.4, orig.ident) %>% summarise(ncells = n())
...
plt <- ggplot(proportions_df, aes(x = orig.ident, y = proportion, fill = `integrated_snn_res.0.4`)) +
...
plt
ggsave(plt, filename = "../../04_results/qc/redo-qc-with-seeds-update-integration/cells-per-cluster-integrated-snn-res-0.4.png")
ggsave(plt, filename = "../../04_results/qc/redo-qc-with-seeds-update-integration/cells-per-cluster-integrated-snn-res-0.4.svg")
ggsave(plt, filename = "../../04_results/qc/redo-qc-with-seeds-update-integration/cells-per-cluster-integrated-snn-res-0.4.pdf")
...
saveRDS(integrated_seurat, file = "../../03_processed_data/03-clustering/integrated_seurat_object_clustered_dietseurat_redo-integration-20251128.Rds")
